    ...
    <el-table :data="cases" stripe>
      ...
      <el-table-column fixed="right" label="操作" width="300">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="excuse(scope.row)"
            >执行</el-button
          >
          <el-button type="danger" size="small" @click="intoEditCase(scope.row)"
            >编辑</el-button
          >
...
export default {
  ...
  methods: {
    ...
    addCase() {
      ...
      axios
        .post("http://127.0.0.1:8001/api/case/" + this.pro_id, addCaseData)
        .then((response) => {
          this.submitUpload();
    ...
    // 获取对应项目下的所有用例
    getCaseList(pro_id) {
      axios.get("http://127.0.0.1:8001/api/case/" + pro_id).then((response) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.data.length > 0) {
          this.pro_id = response.data.data[0].id;
          this.getCaseList(this.pro_id);
        } else {
          this.pro_id = "";
        }
